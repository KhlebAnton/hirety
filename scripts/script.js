...
document.getElementById('feedbackForm').addEventListener('submit', function (event) {
    ...
    if (phoneValue.length === 11) {
        ...
        phoneInput.classList.remove('error-outline');
        phoneError.style.display = 'none';
        closeModal(); // Предполагается, что эта функция существует
    } else {
...
const menuLinks = document.querySelectorAll('.btn_menu-mobile');

menuLinks.forEach((link)=> {
    link.addEventListener('click', ()=> {
        menuLinks.forEach((e) => e.classList.remove('btn_primary'));
        link.classList.add('btn_primary');
